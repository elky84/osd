...
namespace FlatBuffers.Protocol.Response
{
  public struct ShowConfirmDialog : IFlatbufferObject
  {
    ...
    public static byte[] Bytes(string message, string icon) {
      ...
      using (var mstream = new MemoryStream())
      {
        using (var writer = new BinaryWriter(mstream))
        {
          ...
          writer.Write((byte)(typeof(ShowConfirmDialog).FullName.Length));
          writer.Write(Encoding.Default.GetBytes(typeof(ShowConfirmDialog).FullName));
          writer.Write(bytes);
          writer.Flush();
    ...
    public static byte[] Bytes(Model model) {
      return Bytes(model.Message, model.Icon);
    }
  };
}