...
namespace FlatBuffers.Protocol
{
  public struct Items : IFlatbufferObject
  {
    ...
    public struct Model
    {
      public List<FlatBuffers.Protocol.Item.Model> Inventory { get; set; }
      public List<FlatBuffers.Protocol.Equipment.Model> Equipment { get; set; }
    
      public Model(List<FlatBuffers.Protocol.Item.Model> inventory, List<FlatBuffers.Protocol.Equipment.Model> equipment)
      {
        Inventory = inventory;
        Equipment = equipment;
      }
    ...
    public static byte[] Bytes(List<FlatBuffers.Protocol.Item.Model> inventory, List<FlatBuffers.Protocol.Equipment.Model> equipment) {
      var builder = new FlatBufferBuilder(512);
      var inventoryOffset = CreateInventoryVector(builder, inventory.Select(x => FlatBuffers.Protocol.Item.CreateItem(builder, x.Id, builder.CreateString(x.Name))).ToArray());
      ...
      using (var mstream = new MemoryStream())
      {
        using (var writer = new BinaryWriter(mstream))
        {
          ...
          writer.Write((byte)(nameof(Items).Length));
          writer.Write(Encoding.Default.GetBytes(nameof(Items)));
          writer.Write(bytes);
          writer.Flush();
